"""
TeroAI - Daily Outreach Script (Batch 2: leads 101-200)
Processes BATCH_SIZE leads per run, saves progress, creates Gmail drafts.
Usage:
  python outreach_daily2.py                  # resumes from last saved row
  python outreach_daily2.py --start-row 100  # force start at row 101 (0-indexed: 100)
Set before running:
...
PROGRESS_FILE = os.path.expanduser("~/.teroai_progress2.json")
...
def main():
    ...
    total = len(all_rows)
    if start_row >= total:
        print("All leads processed. Delete ~/.teroai_progress2.json to start over.")
        return
